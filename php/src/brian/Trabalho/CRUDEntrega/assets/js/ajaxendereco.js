...
function enviaDados() {
    ...
    if (verificaTamanho($('#endereco').val(), 3, 'descrição') ||
            verificaTamanho($("#endtipo").val(), 3, 'rua')) {
        ...
        if (sCodigo) {
            sUrl = 'endereco/alterar.php';
        } else {
            sUrl = 'endereco/inserir.php';
            sCodigo = '';
        }
...
function selecionar() {
    ...
    if (aSelect.length > 1) {
        ...
        $.ajax({
            type: 'POST',
            url: 'endereco/selecionar.php',
            data: {'codigo': aSelect[0]},
            success: function (data) {
                var aEnderecos = JSON.parse(data);
                $('#codigo').val(aEnderecos[0]);
                $('#endtipo').val(aEnderecos[1]);
                $('#endereco').val(aEnderecos[2]);
            }
        });
    }
}
